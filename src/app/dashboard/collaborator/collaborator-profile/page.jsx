        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-zinc-400 text-sm mt-1">
              Manage your personal branding, skills, and resume details.
            </p>
          </div>
          {!isEditing && (
            <button
        ...
        {!isEditing ? (
          <div className="bg-zinc-900/40 border border-zinc-800/85 p-6 md:p-8 rounded-3xl backdrop-blur-md shadow-2xl space-y-8">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 pb-6 border-b border-zinc-800/60">
              <div className="relative w-28 h-28 rounded-full overflow-hidden border-2 border-indigo-500/50 shadow-lg shrink-0">
                {profile?.image ? (
                  <img
                    src={profile.image}
                    alt={profile.name}
            ...
            <div className="space-y-3">
              ...
              <div className="bg-zinc-950/50 border border-zinc-800/60 p-4 rounded-2xl text-zinc-300 text-sm leading-relaxed whitespace-pre-wrap">
                {profile?.bio || (
                  <span className="text-zinc-600 italic">No bio added yet. Click &quot;Edit Profile&quot; to write about yourself!</span>
                )}
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2.5">
                {profile?.skills && profile.skills.length > 0 ? (
                  profile.skills.map((skill, index) => (
                    <span
                      key={index}
                      className="px-3.5 py-1.5 rounded-full text-xs font-medium bg-zinc-850 text-zinc-200 border border-zinc-800 hover:border-zinc-700 transition-colors"
                    >
                      {skill}
                    </span>
                  ))
          ...
          <form onSubmit={handleSave} className="bg-zinc-900/40 border border-zinc-800/85 p-6 md:p-8 rounded-3xl backdrop-blur-md shadow-2xl space-y-6">
            ...
            <div className="flex flex-col items-center sm:flex-row gap-6 pb-6 border-b border-zinc-800/60">
              <div className="relative w-28 h-28 rounded-full overflow-hidden border-2 border-indigo-500/50 shadow-lg shrink-0 bg-zinc-950">
                {preview ? (
                  <img
                    src={preview}
                    alt="Preview"
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                placeholder="Tell others about yourself, your goals, and what you build..."
                rows={4}
                className="w-full px-4 py-2.5 bg-zinc-950 border border-zinc-800 rounded-xl text-white text-sm focus:outline-none focus:border-indigo-500 transition-all resize-none"
              />
            </div>
